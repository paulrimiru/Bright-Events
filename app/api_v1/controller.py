...
class Controller(object):
    ...
    def add_event(self, event_data):
        ...
        creates an event
        """
        resp = self.events.create_event(event_data)
        if resp.get('success'):
            return {'success':True, 'payload':event_data}
        return {'success':False, 'message':resp.get('message')}
    ...
    def retrieve_single_event(self, user_id, event_id):
        ...
        if resp.get("success"):
            return {'success':True, "payload":resp.get("message")}
        return {'success':False, "message":resp.get("message")}
    def delete_single_event(self, email, eventname):
        """
        deletes a single event
        """
        resp = self.events.delete_event(email, eventname)
        if resp.get('success'):
            return {'success':True, 'payload':resp.get('message')}
        return {'success':False, 'message':resp.get('message')}
    ...
    def retrieve_rsvp(self, user_id, event_id):
        ...
        if resp.get('success'):
            return {'success':True, 'payload':resp.get('message')}
        return resp
    def retrieve_events_by_name(self, name):
        """
    ...
    def edit_event(self, email, name, newevent):
        """
        edits a specific event
        """
        resp = self.events.edit_event(email, name, newevent)
        if resp.get('success'):
            return {'success':True, 'payload':resp.get('message')}
        return {'success':False, 'message':resp.get('message')}
    def accept_rsvp(self, user_id, event_id, clientemail):
        """
        accepts a specific rsvp
        """
        resp = self.events.confirm_rsvp(user_id, event_id, clientemail)
        if resp.get('success'):
            return {'success':True, 'payload':resp.get('message')}
        return {'success':False, 'message':resp.get('message')}
    def reject_rsvp(self, user_id, event_id, clientemail):
        """
        rejects a specific rsvp
        """
        resp = self.events.reject_rsvp(user_id, event_id, clientemail)
        if resp.get('success'):
            return {'success':True, 'payload':resp.get('message')}
        return {'success':False, 'message':resp.get('message')}  